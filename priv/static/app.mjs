...
// build/dev/javascript/app/pokemon.mjs
...
var UserTypedPokemon = class extends CustomType {
  constructor($0) {
    super();
    this[0] = $0;
  }
...
function view_new_pokemon(new_pokemon) {
  return div(
    toList([class$("bg-ctp-base rounded-b-sm p-2 flex gap-2")]),
    toList([
      input(
        toList([
          class$(
            "placeholder:text-ctp-overlay1 focus:outline-none border-1 border-ctp-overlay1 rounded-sm p-2 flex-1 text-ctp-text w-full"
          ),
      ...
      button(
        toList([
          class$(
            "hover:bg-ctp-green-500 hover:text-ctp-base hover:cursor-pointer p-2 rounded-sm border-1 border-ctp-overlay1 text-ctp-overlay1 "
          ),
...
function view_pokemon_list(pokemon_list) {
  return div(
    toList([
      class$(
        "bg-ctp-base grid grid-cols-3 grid-rows-4 gap-2 sm:grid-cols-5 sm:grid-rows-5 md:grid-cols-6 md:grid-rows-8 rounded-t-sm p-2"
      )
    ]),
...
function main() {
  ...
  if (!($ instanceof Ok)) {
    throw makeError(
      ...
      { value: $, start: 832, end: 904, pattern_start: 843, pattern_end: 848 }
    );
  }
  return void 0;
}
